...
public class DemoApplication {
	...
	List<String> forthClue = new ArrayList<String>(List.of("0, true",
			"1, true",
			"0, false",
			"0, true",
			"1, true",
	...
	public ResponseEntity<PuzzleResponseEntity> createEscape(@RequestBody CreateEscapeRequest request){
		EscapeEntity escape = new EscapeEntity(request.year(), request.name(), request.gameIsOut());

		if(	escape.isCorrectValues()) {
			return ResponseEntity.status(HttpStatus.OK).body(new PuzzleResponseEntity( "Post puzzle, correct!", 2,'J'));
		}else{
			return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(new PuzzleResponseEntity( "Incorrect", 1,'_'));
		}
	}

	@PutMapping("/api/escape")
	public  ResponseEntity<PuzzleResponseEntity> changeSequence(@RequestParam int index, @RequestParam int value){
		if(index == 5 && value == 32){
			return ResponseEntity.status(HttpStatus.OK).body(new PuzzleResponseEntity( "Put puzzle, correct!", 3,'Y'));

		}else {
			return ResponseEntity.status(HttpStatus.EXPECTATION_FAILED).body(new PuzzleResponseEntity( "Incorrect", 3,'_'));
		}
	}
	...
	public ResponseEntity<SolutionEntity> solution(@PathVariable String code){
		...
		if(code.equals(finalCode)) {
			return ResponseEntity.status(HttpStatus.OK).body(new SolutionEntity("""
					Congratulations!
